...
def render_markdown_with_includes(content: str, source_file: Path) -> str:
    ...
    def replace_fenced(match: re.Match[str]) -> str:
        ...
        include_path = (source_file.parent / include_expr).resolve()
        if not include_path.is_file():
            raise DocsPipelineError(f"Include path does not exist for {source_file}: {include_expr} -> {include_path}")
...
def prepare_docs_tree(source_dir: Path, output_dir: Path) -> list[Path]:
    ...
    for source_file in sorted(source_dir.rglob("*")):
        ...
        if source_file.suffix.lower() in MARKDOWN_SUFFIXES:
            ...
            rendered = render_markdown_with_includes(original, source_file)
...
def run_zensical(
    *,
    ...
    try:
        ...
        raise DocsPipelineError(f"Zensical command failed with exit code {exc.returncode}: {' '.join(cli)}") from exc
